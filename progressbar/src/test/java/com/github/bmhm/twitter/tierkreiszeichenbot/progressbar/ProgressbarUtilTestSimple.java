...
class ProgressbarUtilTestSimple {

  private static final Logger LOG = LoggerFactory.getLogger(ProgressbarUtilTestSimple.class);

  @Test
  void getRepresentation_empty() {
    final int barLength = 16;
    final ProgressType blockSimple = ProgressType.BLOCK_SIMPLE;
  ...
  void getRepresentation_anyLength_half() {
    ...
    final Set<ProgressbarUtil> progressbarUtils = IntStream.range(0, 40)
        .mapToObj(length -> new ProgressbarUtil(blockSimple, length))
        .collect(toSet());

    progressbarUtils.forEach(bar -> {
